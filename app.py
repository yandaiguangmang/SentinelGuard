...
def sentinel_analyze():
    """SentinelGuard 统一安全检测入口。"""
    try:
        data = request.get_json(silent=True) or {}
        target = (data.get('target') or '').strip()
        target_type = (data.get('target_type') or 'auto').strip()
        fetch_page = bool(data.get('fetch_page', True))

        report = run_static_detection(target, target_type=target_type, fetch_page=fetch_page)
        return jsonify({
            'success': True,
            'result': report.to_dict()
        })
    except Exception as exc:
        logger.exception(f"SentinelGuard检测失败: {exc}")
        return jsonify({
            'success': False,
            'error': f'检测失败: {exc}'
        }), 500
...
@app.route('/api/sentinel/deep-analyze', methods=['POST'])
def sentinel_deep_analyze():
    """SentinelGuard URL 模型深度检查入口。"""
    try:
        data = request.get_json(silent=True) or {}
        target = (data.get('target') or '').strip()
        target_type = (data.get('target_type') or 'auto').strip()
        ...
        static_report = run_static_detection(target, target_type=target_type, fetch_page=fetch_page)
        deep_report = run_deep_url_detection_from_static(static_report)
        return jsonify({
            'success': True,
            'static_result': static_report.to_dict(),
            'deep_result': deep_report.to_dict()
        })
    except Exception as exc:
        logger.exception(f"SentinelGuard深度检查失败: {exc}")
        return jsonify({
            'success': False,
            'error': f'深度检查失败: {exc}'
        }), 500

@app.route('/sentinel_reports/<path:filename>')
def sentinel_report_file(filename):
    """SentinelGuard 报告文件访问入口。"""
    from flask import send_from_directory
    return send_from_directory('sentinel_reports', filename)
